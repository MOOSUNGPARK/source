...
# 구글의 이미지 검색에 해당하는 input 창의 id 가 'nx_query' 임(검색창에 해당하는 html코드를 찾아서 elem 사용하도록 설정)
...
def fetch_list_url():
    params = []
    imgList = soup.find_all("img", class_="rg_ic rg_i")  # 구글 이미지 url 이 있는 img 태그의 _img 클래스에 가서
    for im in imgList:
        try :
