...
import numpy as np
from PythonClass.cifar_10.layers import *
...
class TwoLayerNet:
    def __init__(self, input_size, hidden_size, output_size, weight_init_std=0.01):
        # 가중치 초기화
        self.params = {}
        self.params['W1'] = np.sqrt(2/input_size) * np.random.randn(input_size, hidden_size)  # 표준 정규 분포를 따르는 난수 생성
        self.params['b1'] = np.zeros(hidden_size)
        self.params['W2'] = np.sqrt(2/hidden_size) * np.random.randn(hidden_size, hidden_size)
        self.params['b2'] = np.zeros(hidden_size)
        self.params['W3'] = np.sqrt(2/hidden_size) * np.random.randn(hidden_size, hidden_size)
        self.params['b3'] = np.zeros(hidden_size)
        self.params['W4'] = np.sqrt(2/hidden_size) * np.random.randn(hidden_size, hidden_size)
        self.params['b4'] = np.zeros(hidden_size)
        self.params['W5'] = np.sqrt(2/hidden_size) * np.random.randn(hidden_size, output_size)
