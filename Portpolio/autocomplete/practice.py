...
class Model:
    def __init__(self, session, in_size, hidden_size, layers_cnt, out_size, lr=0.001, name='model'):
        self.scope = name
        self.sess = session
        ...
        self.lr = lr
        self.state_size = layers_cnt * 2 * hidden_size
    ...
    def _build_net(self):
        ...
        def add_fclayer(name, l, hidden_size, out_size, out_layer=False):
        ...
        def lstm_model():
            outputs, new_state = add_lstmlayer('LSTM')
            output_shape = tf.shape(outputs)
            outputs = tf.reshape(outputs, [-1, self.hidden_size])
            logits = add_fclayer('FC_out', outputs, self.hidden_size, self.out_size, out_layer=True)
    ...
    def generating_test(self, x_data, init_state=True):
        if init_state:
