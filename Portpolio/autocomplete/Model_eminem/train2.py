...
import time
import numpy as np
import Portpolio.autocomplete.Model_eminem.util2 as txt
...
c_to_i_vocab, i_to_c_vocab, codetext, valitext, bookranges = txt.read_data_files(shakedir, validation=True)
vocab_size = len(c_to_i_vocab)
...
# inputs
X = tf.placeholder(tf.float32, [None, None, vocab_size], name='X')    # [ BATCHSIZE, SEQLEN ]
# Xo = tf.one_hot(X, vocab_size, 1.0, 0.0, axis=-1)                 # [ BATCHSIZE, SEQLEN, VOCABSIZE ]
# expected outputs = same sequence shifted by 1 since we are trying to predict the next character
Y_ = tf.placeholder(tf.float32, [None, None, vocab_size], name='Y_')  # [ BATCHSIZE, SEQLEN ]
# Yo_ = tf.one_hot(Y_, vocab_size, 1.0, 0.0, axis=-1)               # [ BATCHSIZE, SEQLEN, VOCABSIZE ]
# input state
...
w = tf.get_variable(name='W', shape=[INTERNALSIZE, vocab_size], dtype=tf.float32,
                    initializer=layers.variance_scaling_initializer())
b = tf.Variable(tf.constant(value=0.001, shape=[vocab_size]))
Yflat = tf.reshape(Yr, [-1, INTERNALSIZE])    # [ BATCHSIZE x SEQLEN, INTERNALSIZE ]
Ylogits = tf.matmul(Yflat, w) + b
# Ylogits = layers.linear(Yflat, vocab_size)     # [ BATCHSIZE x SEQLEN, VOCABSIZE ]
Yflat_ = tf.reshape(Y_, [-1, vocab_size])     # [ BATCHSIZE x SEQLEN, VOCABSIZE ]
loss = tf.nn.softmax_cross_entropy_with_logits(logits=Ylogits, labels=Yflat_)  # [ BATCHSIZE x SEQLEN ]
loss = tf.reshape(loss, [batchsize, -1])      # [ BATCHSIZE, SEQLEN ]
Yo = tf.nn.softmax(Ylogits, name='Yo')        # [ BATCHSIZE x SEQLEN, VOCABSIZE ]
...
accuracy = tf.reduce_mean(tf.cast(tf.equal(tf.cast(tf.argmax(Y_,axis=-1) , tf.uint8), tf.cast(Y, tf.uint8)), tf.float32))
...
# training loop
for x, y_, epoch in txt.rnn_minibatch_sequencer(codetext, BATCHSIZE, SEQLEN, EPOCHS, vocab_size):
    # train on one minibatch
    feed_dict = {X: x, Y_: y_, Hin: istate, lr: learning_rate, pkeep: dropout_pkeep, batchsize: BATCHSIZE}
    _, y, ostate = sess.run([train_step, Y, H], feed_dict=feed_dict)
    # aaa = sess.run([Xo], feed_dict={X:x})
    # print('x',x)
    # print('xo',aaa)
    ...
    if step % _50_BATCHES == 0:
        ...
        txt.print_learning_learned_comparison(i_to_c_vocab, x, y, BATCHSIZE, SEQLEN, l, bookranges, bl, acc, epoch_size, step, epoch)
    ...
    if step % _50_BATCHES == 0 and len(valitext) > 0:
        ...
        vali_x, vali_y, _ = next(txt.rnn_minibatch_sequencer(valitext, bsize, VALI_SEQLEN, 1, vocab_size))  # all data in 1 batch
    ...
    if step // 5 % _50_BATCHES == 0:
        ...
        # ry = np.array([[c_to_i_vocab["시"]]])

        ry = txt.txt_one_hot(np.array([[c_to_i_vocab['E']]]), vocab_size)
        ry = np.reshape(ry,[1,1,vocab_size])
        ...
        for k in range(GENERATION_SIZE):
            ...
            rc = txt.sample_from_probabilities(vocab_size, ryo, topn=10 if epoch <= 1 else 2)
            print(i_to_c_vocab[rc], end="")
            # ry = np.array([[rc]])
            ry = txt.txt_one_hot(np.array([[rc]]), vocab_size)
            ry = np.reshape(ry,[1,1,vocab_size])
