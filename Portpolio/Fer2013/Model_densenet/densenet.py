    ...
    def _build_graph(self):
        ...
        def add_layer(name, l, block_number, idx):
            with tf.variable_scope(name):
                ...
                c = self.parametric_relu(c, 'Bottle' + str(block_number) + '_' + str(idx))
                c = conv(c, 1, 4 * self.Growth_Rate, 1)
                ...
                c = self.parametric_relu(c, 'Dense' +  str(block_number) + '_' + str(idx))
                c = conv(c, 3, self.Growth_Rate, 1)
                c = dropout(inputs=c, keep_prob=self.dropout_rate, is_training=self.training)
        ...
        def dense_net():
            ...
            with tf.variable_scope('dense_block1'):
                for idx in range(self.N):
                    l = add_layer('dense_layer1_{}'.format(idx), l, 1, idx)
            ...
            l = avg_pool2d(inputs=l, kernel_size=[12,12], stride=1, padding='VALID') # kernel_size 변경
