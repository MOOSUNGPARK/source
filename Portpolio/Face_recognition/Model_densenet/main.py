...
from Portpolio.Fer2013.Model_densenet.densenet8 import Model
...
train_file = 'c:\\python\\source\\Portpolio\\Fer2013\\data\\train_data.csv'
test_file = 'c:\\python\\source\\Portpolio\\Fer2013\\data\\test_data.csv'
...
def monitor_train_cost():
    ...
    plt.title('DenseNet-BC on fer2013')
    plt.legend(loc=1)
    plt.xlabel('Epoch')
    plt.ylabel('Value')
    plt.grid(True)
...
with tf.Session() as sess:
    ...
    for epoch in range(epochs):
        ...
        if validation_loss < best_loss_val:
            best_loss_val = deepcopy(validation_loss)
            check_since_last_progress = 0
            best_model_params = get_model_params()
            saver.save(sess, 'log/densenet_fer2013.ckpt')
        else:
