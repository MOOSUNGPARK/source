...
class Mobilenet(object):
    def __init__(self, is_training=True):
        ...
        if is_training:
            self.labels = tf.placeholder(tf.float32, [None, self.cell_size, self.cell_size, 5 + self.num_class], name='labels')
            ...
            with slim.arg_scope([slim.conv2d, slim.separable_convolution2d, slim.fully_connected],
                ...
                with slim.arg_scope([slim.batch_norm],
                    ...
                    with slim.arg_scope([slim.dropout],
                        ...
                        l = convlayer('conv0', self.images, 32, self.width_multiplier, 2, dropout_yn=True)
                        # print('0', l)
                        l = depthwise_convlayer('conv_dw1', l, 64, self.width_multiplier, 2, dropout_yn=True)
                        # print('1', l)
                        l = depthwise_convlayer('conv_dw2', l, 128, self.width_multiplier, 1)
                        # print('2', l)
                        l = depthwise_convlayer('conv_dw3', l, 128, self.width_multiplier, 2, dropout_yn=True)
                        # print('3', l)
                        l = depthwise_convlayer('conv_dw4', l, 256, self.width_multiplier, 1)
                        # print('4', l)
                        l = depthwise_convlayer('conv_dw5', l, 256, self.width_multiplier, 2, dropout_yn=True)
                        # print('5', l)
                        l = depthwise_convlayer('conv_dw6', l, 512, self.width_multiplier, 1)
                        # print('6', l)
                        ...
                        # print('14', l)
                        # l = tf.reshape(l, shape=[-1, int(7 * 7 * 256 * self.width_multiplier)])
                        # print('15', l)

                        l = tf.transpose(l, [0, 3, 1, 2], name='trans')
                        l = slim.flatten(l, scope='flatten')

                        l = fclayer('fc14', l, 4096, dropout_yn=True)
                        l = fclayer('fc15', l, self.output_size, out_layer=True)
                        # print('16', l)
        return l
