...
import numpy as np
from Portpolio.Dog_Cat.Model_vggnet.renewal2.util import read_data
...
with tf.Session(config=config) as sess:
    ...
    for idx in range(0, len(cfg.TRAIN_FILE_LIST), 2):
        ...
        for start_idx in range(0, len(validation_x), cfg.BATCH_SIZE):
            ...
            feed_dict = {'X_data:0': validation_x_batch, 'Y_data:0': validation_y_batch, 'training:0': False}

            validation_acc = sess.run('accuracy:0' ,feed_dict=feed_dict)
    ...
    for idx in range(0, len(cfg.TEST_FILE_LIST), 2):
        ...
        for start_idx in range(0, test_total_size, cfg.BATCH_SIZE):
            ...
            feed_dict = {'X_data:0': test_x_batch, 'Y_data:0': test_y_batch, 'training:0': False}


            acc = sess.run('accuracy:0' ,feed_dict=feed_dict)
